...
export default function App() {
  ...
  const [sortBy, setSortBy] = useState('rating'); // 'rating', 'price', 'date'
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (currentShop) {
        const docRef = doc(db, 'artifacts', appId, view === 'public' ? 'public' : 'users', view === 'public' ? 'data' : user.uid, 'coffee_shops', currentShop.id);
        await updateDoc(docRef, shopData);
      } else {
        const collectionRef = collection(db, 'artifacts', appId, view === 'public' ? 'public' : 'users', view === 'public' ? 'data' : user.uid, 'coffee_shops');
      ...
      <header className="bg-white border-b border-stone-200 sticky top-0 z-30 px-4 py-4 shadow-sm">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {user?.isAnonymous ? (
              <button onClick={handleGoogleLogin} className="p-2 text-stone-400 hover:text-stone-900"><LogIn size={20}/></button>
            ) : (
        ...
        <div className="bg-white p-4 rounded-3xl border border-stone-200 shadow-sm flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-stone-50 border border-stone-100 py-3 pl-12 pr-4 rounded-2xl outline-none focus:ring-2 focus:ring-amber-700/10 transition-all"
          ...
          <div className="flex gap-2">
            <div className="bg-stone-50 border border-stone-100 p-1 rounded-2xl flex items-center overflow-hidden">
              <select 
                className="bg-transparent text-xs font-bold uppercase px-3 py-2 outline-none appearance-none pr-8 relative cursor-pointer"
              ...
              <button 
                ...
                className="p-2 hover:bg-white rounded-xl transition-colors text-amber-800"
              >
                <ArrowUpDown size={16} />
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-stone-100 flex justify-between items-center bg-stone-50">
              ...
              <button onClick={() => setIsModalOpen(false)} className="bg-white p-2 rounded-full border border-stone-200 hover:bg-stone-50 shadow-sm"><X size={20}/></button>
            </div>
            <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase text-stone-400 mb-2 ml-1">Coffee Shop Search</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-300" size={18}/>
                  <input ref={inputRef} type="text" placeholder="Search Google Maps..." className="w-full bg-stone-50 border border-stone-200 py-3 pl-10 pr-4 rounded-xl outline-none focus:ring-2 focus:ring-amber-700/10" defaultValue={currentShop?.name} />
                </div>
                {formData.name && <p className="mt-2 text-xs font-black text-amber-700 uppercase tracking-tighter">✓ Selected: {formData.name}</p>}
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-stone-50 rounded-2xl border border-stone-100">
                <div className="text-xs font-black uppercase text-stone-800 flex items-center gap-2"><Snowflake size={16} className="text-blue-500"/> Cold Brew on menu?</div>
              ...
              <textarea rows="3" value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full bg-stone-50 border border-stone-200 p-4 rounded-xl outline-none resize-none focus:ring-2 focus:ring-amber-700/10 text-sm" placeholder="Any specific highlights?"></textarea>
              
              <button type="submit" disabled={isSaving} className={`w-full flex items-center justify-center gap-2 text-white py-4 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-[0.98] ${isSaving ? 'bg-stone-400' : 'bg-amber-700 hover:bg-amber-800 shadow-xl shadow-amber-700/20'}`}>{isSaving ? <Loader2 className="animate-spin" size={20}/> : <Save size={20}/>} {currentShop ? 'Update Entry' : 'Add to List'}</button>
            </form>
          </div>
        </div>
      )}
